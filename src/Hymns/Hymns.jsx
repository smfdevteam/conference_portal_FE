...
export default function Hymns() {
    ...
    const fetchSearchResults = () => {
        setSearchResults(()=>{
            const results = JSON.parse(localStorage.getItem("hymns")).filter(item => item.title.startsWith(searchQuery))
            return results;
        });
    };
    ...
    const getHymnDetails = (indexOfHymn)=>{
        ...
        setSearchQuery('')
        setHymnDetails(()=>{
            const result = searchResults.filter((item , index )=> index == indexOfHymn)
    ...
    return <>
        <div className='text-center'>
            <p className='text-small text-foreground-600 font-semibold'>عندنا حوالى اكتر من ٩٠٠٠ ترنيمة ممكن تشوفهم من غير ما يكون عندك انترنت</p>
            <p className='text-tiny text-red-400 font-medium'>بس عشان العدد كبير، ممكن البحث ياخد وقت شوية</p>
        </div>
